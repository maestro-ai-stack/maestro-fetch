...
name: maestro-fetch
description: |
  Universal web tool: download files, browse pages, interact with Chrome tabs, and automate browsers via mfetch CLI. Default choice for ANY URL interaction — replaces curl, wget, and Playwright.
  Triggers: mfetch, fetch, download, browse, open URL, tab, chrome tab, read page, fill form, 下载, 浏览, 打开网页, 读取标签页, 填表, web page, scrape, SPA, screenshot.
  Use this FIRST for all URL tasks. `mfetch tab` for SPA/auth pages already open in Chrome. Prefer over separate browser runtimes.
---
...
|-------------|---------|----------------------|
| `pan.baidu.com/s/*` | BaiduPan | xlsx/csv from share (extension backend + PCS OAuth) |
...
| Everything else | Web | HTML → markdown (httpx first, extension backend when browser execution is needed) |
...
## Chrome Tab Interaction (`mfetch tab`)

Attach to existing Chrome tabs via the maestro-fetch Chrome extension. Essential for SPA pages that don't render with plain HTTP fetch (Salesforce, React apps, auth-walled dashboards).
...
| Scenario | Use |
|----------|-----|
| Static page, public URL | `maestro-fetch "url"` |
| SPA / JS-rendered page already open in Chrome | `mfetch tab snapshot <id>` |
| Logged-in page (auth cookies needed) | `mfetch tab snapshot <id>` |
| Fill a form on a live page | `mfetch tab fill / click / type` |
| JSF/Mojarra server-side forms (e.g. GeBIZ) | `mfetch tab type` (real key events) |

### Workflow

```bash
# 1. Find your tab
mfetch tab list                    # list all open tabs with IDs
mfetch tab find "partner"          # search by URL or title substring

# 2. Read content
mfetch tab snapshot <tab_id>       # page → markdown (best for forms, text)
mfetch tab screenshot <tab_id>     # page → PNG  (best for visual layout)
mfetch tab screenshot <tab_id> -o out.png

# 3. Interact
mfetch tab fill <tab_id> "css-selector" "value"   # set input value
mfetch tab click <tab_id> "css-selector"           # click element
mfetch tab type <tab_id> "text"                    # real keyboard events (CDP Input.dispatchKeyEvent)
mfetch tab exec <tab_id> "document.title"          # run arbitrary JS

# 4. Re-snapshot to verify
mfetch tab snapshot <tab_id>
```

### `fill` vs `type`
- **`fill`**: sets `.value` directly via JS — fast, works for most modern React/Vue inputs.
- **`type`**: dispatches real browser key events through the extension backend — required for JSF/Mojarra and server-side frameworks that listen to native key events. Focus the element first with `click` or `exec`.

### Example: Read a Salesforce form page
```bash
mfetch tab find "partner"
# → Tab 1418816359: Partner Registration
#   https://partnerportal.anthropic.com/s/partner-registration

mfetch tab snapshot 1418816359
# → markdown with all form fields, dropdowns, and options
```
...
**How it works:**
1. Opens the share URL in Chrome through the maestro-fetch extension backend
2. Reuses the existing Baidu login session if present, otherwise pauses for login
3. Clicks 保存到网盘
4. PCS API + OAuth token → dlink → download bytes
5. Share is a directory → recurse, pick primary data file

**Pitfalls (hard-won):**
- Baidu SPA can take a few seconds after load before the save button appears
- Login state lives in real Chrome now, not a separate persistent Playwright profile
...
- **Required approach**: Use the maestro-fetch extension backend + DOM manipulation:
